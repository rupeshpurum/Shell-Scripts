...
for value in `grep -o 'inventory/.*' "/home/sthummala/workspace/vsure/centina/sa/profiles/$pluginname.xml" | awk -F "/" '{print$2}' | awk -F "\"" '{print$1}' | grep -v 'if-mib.xml\|entity'|xargs -n1 -I {} cat ~/workspace/vsure/centina/sa/profiles/snmp/inventory/{} | sed -e 's/oid="/\noid="/g' -e 's/^.*appends="node.*$//g' -e 's/suffix/\nsuffix/g' | grep -o 'oid="[0-9.]*"\|suffix="[0-9]*"\|<table name=".* \| <table appends=".*\|<column name=.*' | sed -e 's/<table appends/name/g' -e 's/<table name/name/g' -e 's/<column name/column/g' | sed 's/\s*//g'`
do
a=$(echo $value| awk -F "=" '{print$1}')
	...
	then
		...
		index1=.1
			...
			syntax=$( grep 'mib-repository.xml' "/home/sthummala/workspace/vsure/centina/sa/profiles/$pluginname.xml"| awk -F "\"" '{print$2}' | xargs -n1 -I {} cat ~/workspace/vsure/centina/sa/profiles/{} | grep "$columnname"|head -1 | grep -o "type=.*" | awk -F "\"" '{print$2}')
	...
	then
		...
		path=$(pwd)
		#echo "$table"
		cd ~/workspace/vsure/mibs/"$mibdumpfolder"/  #to search in exact plugin mibs
 		#cd ~/workspace/vsure/mibs/ #to search in entire repo mibs
...
sort -V mibdump.txt >"$pluginname".txt
