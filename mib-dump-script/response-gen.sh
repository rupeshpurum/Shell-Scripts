	...
	then
		...
		cd ~/workspace/vsure/mibs/"$mibdumpfolder"/
			...
			index=1
				...
				if check=$(egrep -whr -A1000 "\b$table\b.*OBJECT-TYPE" | head -1000 |awk '/INDEX/, /::=/' | sed "s/$table/shan/"| awk '1;/shan/{exit}' | grep -q shan)
				then
				index=$(egrep -whr -A1000 "\b$table\b.*OBJECT-TYPE" | head -1000 |awk '/INDEX/, /::=/' | sed "s/$table/shan/"| awk '1;/shan/{exit}' |sed 's/,[ ]*[a-z]/\n/g'| sed 's/\s*//g' | sed '/^\s*$/d' |  awk '/{/,/}/' | sed -e 's/{/{\n/g' -e 's/}/\n}/g'| sed '/^\s*$/d' | sed '/^\s*$/d'| sed -e '/{/d' -e '/}/d' -e '/INDEX/d' | grep -vic "shan1" ) #|sed 's/,/\n/g'| sed 's/^$//'  |  grep -vic "${table}" | sed -e '/"$table"/d' -e '/INDEX/d' | grep -c "\b[a-zA-Z]*\b"
				else
				index=0
				fi
			fi
